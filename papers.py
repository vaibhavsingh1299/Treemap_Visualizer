...
class PaperTree(TMTree):
    ...
    def __init__(self, name: str, subtrees: List[TMTree], authors: str = '',
        ...
        tree. In that case, load data about papers from DATA_FIE to build the
        tree.
        ...
        if all_papers:
            x = _load_papers_to_dict(by_year)
            subtrees = _build_tree_from_dict(x)

        TMTree.__init__(self, self.name, subtrees, self.citations)
...
def _load_papers_to_dict(by_year: bool = True) -> Dict:
    ...
    if by_year:
        ...
        with open(DATA_FILE) as csvfile:
            ...
            for line in reader:
                if line['Year'] not in d:
                    d[line['Year']] = {}
                a = line['Category'].split(':')
                helper1(line['Category'], line['Title'], line['Url'],
                        line['Citations'], line['Author'], line['Year'],
                        d[line['Year']], a)
        ...
        with open(DATA_FILE) as csvfile:
            reader = csv.DictReader(csvfile)
            for row in reader:
                k = row['Category'].split(':')

                helper1(row['Category'], row['Title'], row['Url'],
                        row['Citations'], row['Author'], row['Year'], b, k)

        return b


def helper1(category: str, title: str, url: str, citations: str,
            author: str, year: str, z: dict, a: list):

        if len(a) != 0 and a[0] in z:
            z[a[0]] = helper1(category, title, url, citations, author, year,
                              z[a[0]], a[1:])
            return z
        elif len(a) != 0 and a[0] not in z:
            z[a[0]] = {}
            helper1(category, title, url, citations, author, year, z[a[0]],
                    a[1:])

            return z
        elif len(a) == 0:
            z[(url,)] = (author, title, year, category, url, int(citations))
...
def _build_tree_from_dict(nested_dict: Dict) -> List[PaperTree]:
    ...
    if not isinstance(nested_dict, tuple):
        subtrees = []
        for key in nested_dict:
            b = []
            if not isinstance(key, tuple):
                b.extend(_build_tree_from_dict(nested_dict[key]))
                x = PaperTree(key, b)
                subtrees.append(x)

            else:
                y = [PaperTree(nested_dict[key][1], [], nested_dict[key][0],
                               nested_dict[key][4], nested_dict[key][5])]
